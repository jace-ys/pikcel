...
func main() {
	var (
		hostF = flag.String("host", "local-http", "Server host (valid values: local-http, local-grpc)")
	...
	)
	{
		...
		if addr == "" {
			...
			default:
				fmt.Fprintf(os.Stderr, "invalid host argument: %q (valid hosts: local-http|local-grpc)\n", *hostF)
				os.Exit(1)
			}
		}
		timeout = *timeoutF
...
func usage() {
	...
	fmt.Fprintf(os.Stderr, `%s is a command line client for the pikcel API.

Usage:
    %s [-host HOST][-url URL][-timeout SECONDS][-verbose|-v] SERVICE ENDPOINT [flags]

    -host HOST:  server host (local-http). valid values: local-http, local-grpc
